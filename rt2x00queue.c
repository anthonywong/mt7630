				     ...
				     struct txentry_desc *txdesc)
{
	...
	//queue_hex_dump("rt2x00queue_write_tx_data entry->skb->data",entry->skb->data,entry->skb->len);
	/*
	 * Map the skb to DMA.
	 */
	if (test_bit(REQUIRE_DMA, &rt2x00dev->cap_flags))
			       ...
			       bool local)
{
	...
	int ret = 0;
	struct queue_entry_priv_pci *entry_priv=NULL;
	...
	skbdesc->entry = entry;
	entry->skb = skb;
	//printk("rt2x00queue_write_tx_frame set entry skb=%d\n",entry->entry_idx);
	...
	//queue_hex_dump("pTxd", skbdesc->desc, 16);
...
#if 0
		if (queue->qid != QID_RX)
			printk("queue[%d]->length-- = %d\n",queue->qid,queue->length);
#endif		
	}
