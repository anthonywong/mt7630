...
int rt2x00lib_enable_radio(struct rt2x00_dev *rt2x00dev)
{
	...
	{
		reg=0;
		rt2x00dev_pci_register_read(rt2x00dev,i,&reg);
		printk("[0x%x]=0x%x\n",i,reg);
	}
#endif
	/*
...
void rt2x00lib_disable_radio(struct rt2x00_dev *rt2x00dev)
{
	if (!test_and_clear_bit(DEVICE_STATE_ENABLED_RADIO, &rt2x00dev->flags))
		return;
	printk("===>rt2x00lib_disable_radio\n");
	/*
...
void rt2x00lib_rxdone(struct queue_entry *entry, gfp_t gfp)
{
	...
	//hex_dump("entry->skb->data",entry->skb->data,entry->skb->len);
...
#define RT2X00_TASKLET_INIT(taskletname) \
	...
	RT2X00_TASKLET_INIT(tx0damdone_tasklet);
	RT2X00_TASKLET_INIT(tx1damdone_tasklet);
	RT2X00_TASKLET_INIT(tx2damdone_tasklet);
	RT2X00_TASKLET_INIT(tx3damdone_tasklet);
...
EXPORT_SYMBOL_GPL(rt2x00lib_initialize);
